@php
    if ($report->report_type === 'health_records') {
        $healthRecords = \App\Models\HealthRecord::where('recorded_by_id', $report->bhw_id)
        ...
        $pregnancies = \App\Models\Pregnancy::whereMonth('created_at', $report->report_month)
            ->whereYear('created_at', $report->report_year)
            ->with('woman')
            ->get();
    ...
    <div class="print-container">
        ...
        <div class="no-print mb-4 d-flex justify-content-between align-items-center">
            ...
            <div class="btn-group">
                ...
                <a href="{{ route('rhu.bhw-reports.show', $report->id) }}" class="btn btn-outline-secondary">
                    ...
                    <div class="col-md-6">
                        <table class="table table-borderless table-sm mb-0">
                            <tr>
                                ...
                                <td class="fw-medium">{{ $report->bhw->name ?? 'Unknown' }}</td>
                            </tr>
                            <tr>
                                <td class="text-muted">Email:</td>
                                <td>{{ $report->bhw->email ?? 'N/A' }}</td>
                            </tr>
                            <tr>
        ...
        @else
            <div class="card">
                <div class="card-header bg-light">
                    <h6 class="mb-0"><i class="bi bi-file-medical"></i> Pregnancy Records Detail</h6>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                @forelse($pregnancies as $index => $preg)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $preg->created_at->format('M d, Y') }}</td>
                                        <td>
                                            <div class="fw-medium">{{ $preg->woman->name ?? 'Unknown' }}</div>
                                        </td>
                                        <td>{{ $preg->lmp ? $preg->lmp->format('M d, Y') : 'N/A' }}</td>
                                        <td>{{ $preg->edd ? $preg->edd->format('M d, Y') : 'N/A' }}</td>
                                        <td>G{{ $preg->gravida ?? 0 }} P{{ $preg->parity ?? 0 }}</td>
                                        <td>
                                            @if($preg->is_high_risk)
                                                <span class="risk-badge risk-high">HIGH</span>
                                            @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center py-4">
                                            No pregnancy records found for this report period.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
        ...
        <div class="bhw-signature">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p class="mt-2 mb-0 fw-medium">{{ $report->bhw->name ?? 'Unknown BHW' }}</p>
