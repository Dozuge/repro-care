    ...
    <div class="card-header bg-transparent py-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <h5 class="mb-0 fw-700 text-dark">
                <i class="bi bi-person-rolodex me-2" style="color: var(--primary);"></i>Maternal Registry List
            </h5>

            <!-- Search Form -->
            <form method="GET" action="{{ route('rhu.reports.index') }}" class="d-flex gap-2">
                <div class="input-group input-group-sm" style="width: 250px;">
                    <span class="input-group-text bg-light border-end-0"><i class="bi bi-search"></i></span>
                    <input type="text" name="search" class="form-control bg-light border-start-0 ps-0" placeholder="Search by name or barangay..." value="{{ $search }}">
                </div>
                <button type="submit" class="btn btn-primary btn-sm px-3">Filter</button>
                @if($search)
                    <a href="{{ route('rhu.reports.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x-lg"></i></a>
                @endif
            </form>
        </div>
    </div>
    <div class="card-body p-0">
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @foreach($patients as $patient)
                            <tr>
                                <td class="px-4">
                                    <span class="fw-700 text-dark d-block" style="font-size:0.875rem;">{{ $patient->name }}</span>
        ...
        @else
            <div class="text-center py-5">
                ...
                <p class="text-muted text-xs px-4">No active patients registered in the rural health database fit the current filter criteria.</p>
            </div>
        @endif
    </div>
</div>
